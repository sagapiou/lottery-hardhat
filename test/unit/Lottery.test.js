    ...
    : describe("Lottery Unit Tests", function () {
          ...
          describe("enterLottery", function () {
              ...
              it("doesn't allow entrance when lottery is calculating", async () => {
                  await lottery.enterLottery({ value: lotteryEntranceFee })
                  // play with block time and mining on the hardhat network with below 2 lines
                  // in order to test future events
          ...
          describe("checkUpkeep", function () {
              it("returns false if people haven't sent any ETH", async () => {
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.request({ method: "evm_mine", params: [] })
                  //callStatic will just return the value of the function. the transaction will not be sent
          ...
          describe("performUpkeep", function () {
              ...
              it("updates the lottery state and emits a requestId", async () => {
                  ...
                  const lotteryState = await lottery.getLotteryState()
                  const requestId = txReceipt.events[1].args.winner
